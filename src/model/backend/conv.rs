...
impl WorkflowBuilder {
    ...
    pub fn into_workflow(mut self, graph: &Graph) -> Workflow {
        for link in graph.get_links() {
            ...
            let mut src_widgets_values = src_widgets_values.iter().filter_map(|(_, value)| value.state()).collect::<Vec<_>>();
            let mut dst_widgets_values = dst_widgets_values.iter().filter_map(|(_, value)| value.state()).collect::<Vec<_>>();

            // FIXME: Patch for comfyui#3419 where ksampler control_after_generate value is
            // expected but not defined in the input list
            [(&src.ty.0, &mut src_widgets_values), (&dst.ty.0, &mut dst_widgets_values)]
                .iter_mut()
                .filter(|(ref ty, _)| *ty == "KSampler")
                .for_each(|(_, values)| values.insert(1, "fixed".into()));

            let Some(src_id) = self.create_or_get_node(src, link.src_node, src_widgets_values) else { continue; };
            let Some(dst_id) = self.create_or_get_node(dst, link.dst_node, dst_widgets_values) else { continue; };

            self.workflow.link(src_id, dst_id, link.src_slot, link.dst_slot).unwrap();
        }

        self.workflow
    }

    fn create_or_get_node(&mut self, node: &NodeInstance, idx: usize, widgets_values: Vec<Value>) -> Option<usize> {
        ...
        let inputs = meta.inputs.iter().map(|(name, ty)| {
            WorkflowNodeInput {
                name: name.clone(),
                ty: ty.0.clone(),
                link: 0,
        ...
        let outputs = meta.outputs.iter().enumerate().map(|(idx, (name, ty))| {
            WorkflowNodeOutput {
                name: name.clone(),
                ty: ty.0.clone(),
                links: vec![],
